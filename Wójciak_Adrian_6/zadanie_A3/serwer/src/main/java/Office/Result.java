//
// Copyright (c) ZeroC, Inc. All rights reserved.
//
//
// Ice version 3.7.5
//
// <auto-generated>
//
// Generated from file `office.ice'
//
// Warning: do not edit this file.
//
// </auto-generated>
//

package Office;

public class Result implements java.lang.Cloneable,
                               java.io.Serializable
{
    public boolean isResultPositive;

    public Time finalEndTime;

    public String caseType;

    public Result()
    {
        this.finalEndTime = new Time();
        this.caseType = "";
    }

    public Result(boolean isResultPositive, Time finalEndTime, String caseType)
    {
        this.isResultPositive = isResultPositive;
        this.finalEndTime = finalEndTime;
        this.caseType = caseType;
    }

    public boolean equals(java.lang.Object rhs)
    {
        if(this == rhs)
        {
            return true;
        }
        Result r = null;
        if(rhs instanceof Result)
        {
            r = (Result)rhs;
        }

        if(r != null)
        {
            if(this.isResultPositive != r.isResultPositive)
            {
                return false;
            }
            if(this.finalEndTime != r.finalEndTime)
            {
                if(this.finalEndTime == null || r.finalEndTime == null || !this.finalEndTime.equals(r.finalEndTime))
                {
                    return false;
                }
            }
            if(this.caseType != r.caseType)
            {
                if(this.caseType == null || r.caseType == null || !this.caseType.equals(r.caseType))
                {
                    return false;
                }
            }

            return true;
        }

        return false;
    }

    public int hashCode()
    {
        int h_ = 5381;
        h_ = com.zeroc.IceInternal.HashUtil.hashAdd(h_, "::Office::Result");
        h_ = com.zeroc.IceInternal.HashUtil.hashAdd(h_, isResultPositive);
        h_ = com.zeroc.IceInternal.HashUtil.hashAdd(h_, finalEndTime);
        h_ = com.zeroc.IceInternal.HashUtil.hashAdd(h_, caseType);
        return h_;
    }

    public Result clone()
    {
        Result c = null;
        try
        {
            c = (Result)super.clone();
        }
        catch(CloneNotSupportedException ex)
        {
            assert false; // impossible
        }
        return c;
    }

    public void ice_writeMembers(com.zeroc.Ice.OutputStream ostr)
    {
        ostr.writeBool(this.isResultPositive);
        Time.ice_write(ostr, this.finalEndTime);
        ostr.writeString(this.caseType);
    }

    public void ice_readMembers(com.zeroc.Ice.InputStream istr)
    {
        this.isResultPositive = istr.readBool();
        this.finalEndTime = Time.ice_read(istr);
        this.caseType = istr.readString();
    }

    static public void ice_write(com.zeroc.Ice.OutputStream ostr, Result v)
    {
        if(v == null)
        {
            _nullMarshalValue.ice_writeMembers(ostr);
        }
        else
        {
            v.ice_writeMembers(ostr);
        }
    }

    static public Result ice_read(com.zeroc.Ice.InputStream istr)
    {
        Result v = new Result();
        v.ice_readMembers(istr);
        return v;
    }

    static public void ice_write(com.zeroc.Ice.OutputStream ostr, int tag, java.util.Optional<Result> v)
    {
        if(v != null && v.isPresent())
        {
            ice_write(ostr, tag, v.get());
        }
    }

    static public void ice_write(com.zeroc.Ice.OutputStream ostr, int tag, Result v)
    {
        if(ostr.writeOptional(tag, com.zeroc.Ice.OptionalFormat.FSize))
        {
            int pos = ostr.startSize();
            ice_write(ostr, v);
            ostr.endSize(pos);
        }
    }

    static public java.util.Optional<Result> ice_read(com.zeroc.Ice.InputStream istr, int tag)
    {
        if(istr.readOptional(tag, com.zeroc.Ice.OptionalFormat.FSize))
        {
            istr.skip(4);
            return java.util.Optional.of(Result.ice_read(istr));
        }
        else
        {
            return java.util.Optional.empty();
        }
    }

    private static final Result _nullMarshalValue = new Result();

    /** @hidden */
    public static final long serialVersionUID = -173062393L;
}
